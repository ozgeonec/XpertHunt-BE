...
passport.use('local-login', new LocalStrategy({
    ...
    User.findOne({ email:  email },'password', function(err, user) {
        ...
        if (!user.comparePassword(password))
            return done(null, false, console.log("wrong password"), req.flash('loginMessage', 'Oops! Wrong password.')); // create the loginMessage and save it to session as flashdata
